...
class Migration
{
    ...
    public function getOverviewDescription()
    {
        ...
        if ($this->sync_preset == 'all') {
            $desc .= "<br> - " . __("Migrate entire site (database and files)", "wpsynchro");
        } elseif ($this->sync_preset == 'db_all') {
            $desc .= "<br> - " . __("Migrate entire database", "wpsynchro");
        } elseif ($this->sync_preset == 'file_all') {
            $desc .= "<br> - " . __("Migrate all files", "wpsynchro");
        } elseif ($this->sync_preset == 'none') {
            if (!$this->sync_database && !$this->sync_files) {
                $desc .= "<br> - " . __("Custom migration - But no data chosen for sync", "wpsynchro");
            } else {
                $desc .= "<br> - " . __("Custom migration", "wpsynchro");
            }
        }
        ...
        if ($this->sync_files) {
            if (count($this->file_locations) > 0) {
                ...
                if ($this->sync_preset == 'all' || $this->sync_preset == 'file_all') {
                    ...
                    $desc .= "<br> - " . __("Files: No locations chosen for migration", "wpsynchro");
                }
            }
        }
        if ($this->sync_files) {
