...
class RemoteTransport implements RemoteConnection
{
    ...
    public function handleRetries($type, &$wpremoteresult)
    {
        ...
        $wpremoteresult->errors[] = sprintf(__("Could not connect to %s service (HTTP statuscode: %d)", "wpsynchro"), $parsed_host, $wpremoteresult->statuscode);
    }
    ...
    private function doRequest($wpremoteresult)
    {
        ...
        if (is_wp_error($response)) {
            ...
            if ($statuscode === 200) {
                ...
                $wpremoteresult->debugs[] = "Call to service at url " . $this->url . " failed with HTTP error code: " . $statuscode . " - ";
                return false;
            }
        }
    }
