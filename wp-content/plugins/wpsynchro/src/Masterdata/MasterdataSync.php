...
class MasterdataSync
{
    ...
    public function runMasterdataStep(&$migration, &$job)
    {
        ...
        if (count($this->job->errors) == 0) {
            ...
            // Check licensing
            if (\WPSynchro\Utilities\CommonFunctions::isPremiumVersion() && count($this->job->errors) === 0) {
    ...
    public function inititializeConfigurations()
    {
        ...
        $this->job->masterdata_max_memory_limit_bytes = (min($this->job->from_memory_limit, $this->job->to_memory_limit) - memory_get_peak_usage()) * 0.7;
    }
