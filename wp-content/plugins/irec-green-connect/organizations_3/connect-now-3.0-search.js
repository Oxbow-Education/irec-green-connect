...
function generateOrgHTML(item) {
  ...
  <div class="organization__container">
    ...
    <div class="organization__all-tags organization__opportunities">
      ${item.opportunities
        ?.map((opp) => `<p>${opp}</p>`)
        .sort()
        .join('')}
  ...
  <a data-organization="${item.organization_name}" target="_blank" href="${
    item.url || `mailto:${item.email}`
  } " class="organization__connect-now organization__connect-now--mobile" onclick="saveOrgToGa(this)">
        Connect Now
      </a>
