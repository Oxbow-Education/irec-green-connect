...
function onBoundsChanged() {
  ...
  boundsChangeTimeout = setTimeout(() => {
    ...
    orgsSearch.helper
      .setQueryParameter('insideBoundingBox', algoliaBounds)
      .setQueryParameter('aroundLatLng', centerLatLng)
      .search();
...
function handleGeocodeResults(results, status) {
  if (status === 'OK' && results[0]) {
    ...
    const isCity = Boolean(cityComponent);
    const isZip = getAddressComponent('postal_code');

    console.log({ isState, isCity, isZip });
